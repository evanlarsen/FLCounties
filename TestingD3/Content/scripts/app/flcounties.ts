...
/// <reference path="../libs/leaflet/leaflet.d.ts" />
/// <reference path="../libs/topojson/topojson.d.ts" />
declare var d3: any;

import dom = module('app/dom');
...
export var isLoading = ko.observable(true);

export function viewAttached(view: HTMLElement) {
    map = new L.map(view, {
...
function updateLegend(props?) {
    legendContainer.innerHTML = '<h4>Florida Counties</h4>' + (props ?
...
function drawCounties(fl) {
    ...
    var feature = counties.selectAll("path")
        .data(geoJson.geometries)
        .enter().append('path').on('mouseover', countyMouseOver).on('mouseout', countyMouseOut);
